...
class Score_Fusion_Attn(nn.Module):
    def __init__(self):
        super().__init__()

        self.mlp = nn.Sequential(nn.Linear(512 * 4, 64),
                                 nn.ReLU(),
                                 nn.Linear(64, 16),
                                 nn.ReLU(),
                                 nn.Linear(16, 4),
    ...
    def forward(self, nmd, features, logits):
        ...
        #print(b_logits_b_features.shape, n1_logits_n1_features.shape, n2_logits_n2_features.shape, n3_logits_n3_features.shape)
        #sys.exit('break here')

        attn_scores = self.mlp(features.reshape(-1, 512 * 4))
        ...
        #print(b_logits_b_features.mean(), n1_logits_n1_features.mean(), n2_logits_n2_features.mean(), n3_logits_n3_features.mean())
        #sys.exit('break here')
        b_stack = torch.stack([b_logits_b_features,
                             b_logits_n1_features,
                             b_logits_n2_features,
                             b_logits_n3_features],dim=1)
        b_attn = b_stack * attn_scores.unsqueeze(-1)
        ...
        n1_attn = n1_stack * attn_scores.unsqueeze(-1)
        ...
        n2_attn = n2_stack * attn_scores.unsqueeze(-1)
        ...
        n3_attn = n3_stack * attn_scores.unsqueeze(-1)
        ...
        return combined_logits, [combined_logits.cpu(), b_stack.cpu(), n1_stack.cpu(), n2_stack.cpu(), n3_stack.cpu(), attn_scores.cpu()]
