...
class STG(Dataset):
    def __init__(self, data_root, transforms, subset, config):
        ...
        with open(data_root + 'exclude_stg.pkl', 'rb') as f:
            exclude_stg = pickle.load(f)

        self.frame_path_list = glob.glob(data_root + subset + '/*.npy')
        self.frame_path_list = self.filter_classes(self.exclude_examples(self.frame_path_list, exclude_stg, subset))
